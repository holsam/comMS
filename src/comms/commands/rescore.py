...
def run_rescore(input_dir: Path, database: Path, output: Path):
    ...
    if crux_bin is None:
        ...
        raise SystemExit(1)
    log.debug(f'Scanning for Tide-search PSM files in: {input_dir}')
    target_files = sorted(input_dir.glob('*.tide-search.target.txt'))
    if not target_files:
        log.warn(f'No Tide-search target PSM files found in: {input_dir}')
        print(f'[bold red]ERROR:[/bold red] No Tide-search target PSM files found in {input_dir}.')
        raise SystemExit(1)
    log.info(f'Found {len(target_files)} PSM file(s) — starting rescoring')
    ...
    with logging_redirect_tqdm():
        for target_file in tqdm(target_files, desc='Files rescored'):
            ...
            if ok:
                ...
                log.warn(f'Percolator failed for: {target_file.name}')
                n_fail += 1
