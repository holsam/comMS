...
class TestRunQuantify:
    def test_creates_quantify_output_dir(self, crux_bin, pipeline_rescore, tmp_path):
        rescore_dir, fasta = pipeline_rescore
    ...
    def test_spectral_counts_file_exists(self, crux_bin, pipeline_rescore, tmp_path):
        rescore_dir, fasta = pipeline_rescore
        run_quantify(input_dir=rescore_dir, database=fasta, output=tmp_path)
        quantify_dir = tmp_path / 'comms' / 'results' / 'quantify'
        counts_files = list(quantify_dir.glob('*.spectral-counts.txt'))
        assert counts_files, 'No spectral-counts file found after run_quantify'

    def test_prints_quantify_summary(self, crux_bin, pipeline_rescore, tmp_path, capsys):
        rescore_dir, fasta = pipeline_rescore
        run_quantify(input_dir=rescore_dir, database=fasta, output=tmp_path)
        captured = capsys.readouterr()
